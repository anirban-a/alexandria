...
public class BookExchangeService implements IBookExchangeService {
  ...
  public void deleteTransaction(Exchange transaction) {
    ...
    bookExchangeRepository.deleteById(transaction.getId(),
        new PartitionKey(transaction.getFirstPartyId()));
    bookExchangeRepository.deleteById(otherPartyExchangeId,
        new PartitionKey(transaction.getOtherPartyId()));
  }
  ...
  public void markCompleted(String id, String userId) {
    Exchange exchange = bookExchangeRepository.findById(id, new PartitionKey(userId))
        .orElseThrow(
            () -> new ApplicationException(String.format("No such exchange by id %s found", id)));
    ...
    exchangeList.stream().filter(ex -> Objects.nonNull(ex.getFirstPartyBookId()))
        .map(Exchange::getFirstPartyBookId)
