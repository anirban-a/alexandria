...
public class JwtService {
	...
	public String getJwt(final User user) {
		...
		Key secretKey = new SecretKeySpec(key.getBytes(), signatureAlgorithm.getJcaName());
		return Jwts.builder().claim(NAME, String.format("%s %s", user.getFirstName(), user.getLastName()))
				.claim(EMAIL, user.getPrimaryEmail()).setSubject(user.getUsername()).setId(UUID.randomUUID().toString())
				.setIssuedAt(Date.from(now)).signWith(secretKey).setExpiration(Date.from(now.plus(5L, ChronoUnit.DAYS)))
				.compact();
	}
