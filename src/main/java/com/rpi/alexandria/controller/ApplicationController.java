...
public class ApplicationController {
      ...
      if (StringUtils.equals(tokens[0], "Basic")) {
        ...
        if (userService.isValidUser(username, password)) {
          ...
          return ResponseEntity
              .ok(new JWTResponse(jwt, user.getFirstName(), user.getLastName(),
                  user.getUsername()));
        }
      }
  ...
  public ResponseEntity<AppResponse<List<University>>> loadUniversities() {
    ...
    AppResponse<List<University>> response = new AppResponse<>("", OffsetDateTime.now(),
        HttpStatus.OK, "",
        universities);
  ...
  public ResponseEntity<AppResponse> resetToken(@RequestBody StringObj email) {
    ...
    AppResponse appResponse = AppResponse.builder().dateTime(OffsetDateTime.now())
        .httpStatus(HttpStatus.OK)
        .message("Password reset token has been generated for user with spe	cified email address")
        .build();
    return new ResponseEntity<>(appResponse, appResponse.getHttpStatus());
  }
  ...
  public ResponseEntity<AppResponse> validateEmailAccount(
    ...
    log.info("Received change password request...");
    if (userService.updateAccountStatus(validationDetails.getEmail(),
        validationDetails.getValidationCode())) {
      AppResponse appResponse = AppResponse.builder().dateTime(OffsetDateTime.now())
          .httpStatus(HttpStatus.OK)
