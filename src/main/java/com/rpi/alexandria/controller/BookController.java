...
public class BookController extends BaseController {
  ...
  public ResponseEntity<AppResponse<List<BookDTO>>> getBookByISBN(@QueryParam("isbn") String isbn,
    ...
    if (StringUtils.isEmpty(isbn) && StringUtils.isEmpty(name)) {
      AppResponse appResponse = new AppResponse("Both ISBN & book name is empty",
          OffsetDateTime.now(),
    ...
    AppResponse<List<BookDTO>> appResponse = new AppResponse<>("Success", OffsetDateTime.now(),
        HttpStatus.OK, "",
        bookDTOList);
    return new ResponseEntity<>(appResponse, appResponse.getHttpStatus());
  }
  ...
  public ResponseEntity<AppResponse<List<BookDTO>>> getBooksForUser() {
    User user = getUser();
    List<BookDTO> bookDTOList = bookService.findAllBooksForUser(user.getUsername()).stream()
        .map(BookDTO::of)
